...
def init():
    ...
    global translator
    lang_dict = json.load(
        open(os.path.join(basedir, "res/language/language.json"), "r", encoding="UTF-8")
    )
...
def init_settings():
    ...
    if os.path.isfile(file_path) and settingGood:
        ...
        fixdragspeedx, fixdragspeedy = (
            data_params["fixdragspeedx"],
            data_params["fixdragspeedy"],
        )
...
def change_translator(language_code):
    ...
    if language_code == "en_US":
        ...
        translator.load(
            QtCore.QLocale(language_code),
            "langs",
            ".",
            os.path.join(basedir, "res/language/"),
        )

        global TIER_NAMES, HUNGERSTR, FAVORSTR
        TIER_NAMES = [
            translator.translate("others", i) for i in TIER_NAMES
        ]  # .encode('utf-8')
